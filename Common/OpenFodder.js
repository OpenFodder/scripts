var OpenFodder = {
     ...
     */
    createMap: function(pScenario) {
        Session.Reset();

        if(pScenario && pScenario.CreateMap) {
            pScenario.CreateMap();
            return;
        }

        if(typeof MapGen !== "undefined" && MapGen.Integration && MapGen.Integration.GenerateCampaignMap) {
            MapGen.Integration.GenerateCampaignMap();
            return;
        }

        throw new Error("MapGen integration is required to create maps");
    },

    applyPhaseObjectives: function(pPhase) {
        pPhase.ObjectivesClear();
		for( var x = 0; x < Settings.Objectives.length; ++x) {
			pPhase.ObjectiveAdd(Settings.Objectives[x]);
        }
    },
    ...
    createPhase: function(pMissionNumber, pPhaseNumber, pScenario) {
        ...
        this.applyPhaseObjectives(Phase);

        this.createMap(pScenario);
        if(pScenario && pScenario.SyncGeneratedObjectives)
            this.applyPhaseObjectives(Phase);
        Phase.SetAggression(Settings.Aggression.Min, Settings.Aggression.Max);
        ...
        // Custom scenarios may add mission content after creating the terrain.
        var completedScenario = pScenario === Scenario.Random || pScenario === Scenario.Multiplayer;
        if(!completedScenario || !Session.MapGenContext || !Session.MapGenContext.Materialized)
            Validation.ValidateMap();

        if(typeof Engine !== "undefined" && Engine.mapSave)
            Engine.mapSave();
    },
